...
def preprocessing(categorical_feature, numerical_columns, missing_value_columns, user_requirement, llm1):
    prompt = '''
   Given the user requirements, perform the necessary preprocessing steps based on the csv dataset provided, which includes:
...
def code_to_file(code,file_name):
    with open(file_name, "w") as f:
        f.write(code)
        f.close()
def code_execution(file_path,codefile_path):
